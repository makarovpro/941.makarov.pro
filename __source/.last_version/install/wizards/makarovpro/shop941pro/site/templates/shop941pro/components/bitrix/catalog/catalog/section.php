...
$arAvailablePager = array("30", "60", "90");
$show_no = array_search($_REQUEST["show_no"], $arAvailablePager) ? $_REQUEST["show_no"] : $arAvailablePager[0];
?>

    <div class="sort_group group">
    
<div class="sort" style="float:left;">
	<div class="sorttext"><?=GetMessage('SECT_SHOW')?>:</div>
	<?foreach ($arAvailablePager as $val){?>
		<?$className = ($show_no == $val) ? ' current' : '';?>
        <a href="<?=$APPLICATION->GetCurPageParam('show_no='.$val, array('show_no'))?>" class="sortbutton<?=$className?>" rel="nofollow"><span><?=$val?></span></a>
	<?}?>
</div>
...
<div class="sort" style="float:right;">
	...
	<?foreach ($arAvailableSort as $key => $val):
		...
		?>
		<a href="<?=$APPLICATION->GetCurPageParam('sort='.$key.'&order='.$newSort, 	array('sort', 'order'))?>" class="sortbutton<?=$className?>" rel="nofollow"><span><?=GetMessage('SECT_SORT_'.$key)?><?if ($sort == $val[0]):?></span><?endif?></a>
	<?endforeach;?>
</div>
